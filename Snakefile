...
rule sam_conversion:
                ...
                run:
                    shell('java -Xmx8G -jar $PICARD FixMateInformation I= {input.sam} O= {wildcards.sample}.analysis/mapped_reads/{wildcards.sample}.fxd.sam VALIDATION_STRINGENCY=SILENT TMP_DIR=tmp &>> {log}')
...
rule RealignerTargetCreator:
            ...
            shell:
                'java -Xmx8G -jar {config[GATK38_PATH]} -T RealignerTargetCreator -R {input.refgenome} -nt {threads} -I {input.bam} --known {input.site1} -o {output} &>> {log}'
...
rule IndelRealigner:
            ...
            shell:
                'java -Xmx8G -jar {config[GATK38_PATH]} -T IndelRealigner -R {input.refgenome} -I {input.bam} -known {input.site1} --targetIntervals {input.targetIntervals} -o {output} &>> {log}'
...
rule BaseRecalibrator:
                ...
                shell:
                    "java -Xmx8G -jar {config[GATK38_PATH]} -T BaseRecalibrator -R {input.refgenome}"
...
rule PrintReads:
                ...
                shell:
                    "java -Xmx8G -jar {config[GATK38_PATH]} -T PrintReads -R {input.refgenome} -I {input.bam}"
